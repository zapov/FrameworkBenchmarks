...
if ($action -eq 'start') {
	...
	$client = new-object System.Net.WebClient
	$client.DownloadFile( "https://github.com/ngs-doo/revenj/releases/download/1.2.1/http-server.zip", $zip )

	echo "Unzipping HTTP server"
	Unzip $zip $exe
	
	echo "Compiling the server model and downloading DSL Platform compiler..."
	&$java -jar $dslclc temp=$TROOT/tmp/ force dsl=$TROOT/Revenj.Bench manual-json revenj.net=$TROOT/exe/ServerModel.dll no-prompt dependencies:revenj.net=$TROOT/exe download
